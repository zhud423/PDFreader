...
- `bookId` 是内部主键
- `contentHash` 是尽可能稳定的内容身份标识

当前实现里，本地导入直接保存 PDF 内容的 `SHA-256` 到 `contentHash`。

对远程 URL 书源，建议也提供同一语义的值，这样后续跨来源识别才能对齐。
...
当前工程里，`P2` 先按静态 `library.json` 协议落地。
...
- `books[].mimeType`
- `books[].pageCount`
- `books[].firstPageWidth`
- `books[].firstPageHeight`
- `books[].pdfPath`
- `books[].coverPath`（可选）
- `books[].updatedAt`（可选）
...
- `books[].contentHash` 建议直接填写 PDF 文件的 `SHA-256`
- 当前实现不会接受旧字段名 `canonicalKey`
...
- 同来源实例下，远程同步优先按 `[sourceInstanceId + sourceKey]` 识别同一条目
- `contentHash` 作为内容身份，用于跨来源识别与后续去重扩展
- 用户可选择覆盖显示信息或保留两份记录
