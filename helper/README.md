...
- 监听端口 `48321`
- 默认书源协议：HTTPS
- HTTPS 书源端口 `48322`（默认使用）
- 把状态文件写到 macOS 的 `~/Library/Application Support/PDFreaderHelper`

可用环境变量覆盖：

- `PDFREADER_HELPER_PORT`
- `PDFREADER_HELPER_SOURCE_PROTOCOL`（`http`/`https`，默认 `https`）
- `PDFREADER_HELPER_TLS_PORT`
- `PDFREADER_HELPER_ENABLE_TLS`（兼容旧开关；`0` 可关闭 HTTPS）
...
```bash
PDFREADER_HELPER_SOURCE_PROTOCOL=http npm run helper
```
